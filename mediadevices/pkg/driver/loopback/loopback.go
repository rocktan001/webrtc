...
import (
	...
	"github.com/rocktan001/webrtc/malgo"
	"github.com/rocktan001/webrtc/mediadevices/internal/logging"
	...
	"github.com/rocktan001/webrtc/mediadevices/pkg/wave"
)
...
var logger = logging.NewLogger("mediadevices/driver/loopback")
...
func init() {
	...
	devices, err := ctx.Devices(malgo.Loopback)
	if err != nil {
		panic(err)
	}

	for _, device := range devices {
		info, err := ctx.DeviceInfo(malgo.Loopback, device.ID, malgo.Shared)
		if err == nil {
			fmt.Printf("Label %s\n", device.ID.String())
...
func (m *microphone) AudioRecord(inputProp prop.Media) (audio.Reader, error) {
	...
	config.DeviceType = malgo.Loopback
...
func sendempytchunk(m *microphone) {
	for ready == false {
...
func (m *microphone) Properties() []prop.Media {
	...
	// fmt.Printf("supportedProps %#v", supportedProps)
